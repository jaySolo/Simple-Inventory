...
namespace jsolo.simpleinventory.impl.queries
{
    ...
    public class GetPermissionsQueryHandler : IRequestHandler<GetPermissionsQuery, List<PermissionViewModel>>
    {
        private readonly IIdentityDatabaseContext db;

        public GetPermissionsQueryHandler(IIdentityDatabaseContext context)
        {
            db = context;
        }
        ...
        public async Task<List<PermissionViewModel>> Handle(GetPermissionsQuery request, CancellationToken token = default)
        {
            List<PermissionViewModel> getPermissions()
            {
                ...
                return permissions.Select(p => PermissionsRequestsHelpers.MapToViewModel(p)).ToList();
            }
    ...
    public class GetPermissionDetailsQueryHandler : IRequestHandler<GetPermissionDetailsQuery, PermissionViewModel?>
    {
        private readonly IIdentityDatabaseContext db;

        public GetPermissionDetailsQueryHandler(IIdentityDatabaseContext context)
        {
            db = context;
        }
